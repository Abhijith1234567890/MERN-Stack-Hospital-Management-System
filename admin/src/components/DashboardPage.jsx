...
function normalizeDoctor(doc) {
  ...
  return {
    ...
    image,
    appointments,
    earnings: safeNumber(doc.earnings ?? doc.totalEarnings ?? 0, 0),
    raw: doc,
  };
}
...
const DashboardPage = () => {
  ...
  const totals = useMemo(() => {
    ...
    const totalAppointments = safeDoctors.reduce(
      (s, d) => s + safeNumber(d?.appointments?.total, 0),
      0,
    );

    const totalEarnings = safeDoctors.reduce(
      (s, d) => s + safeNumber(d?.earnings, 0),
      0,
    );

    const completed = safeDoctors.reduce(
      (s, d) => s + safeNumber(d?.appointments?.completed, 0),
      0,
    );

    const canceled = safeDoctors.reduce(
      (s, d) => s + safeNumber(d?.appointments?.canceled, 0),
      0,
    );

    const totalLoginPatients = safeDoctors.reduce(
      (s, d) => s + (d?.raw?.loginPatientsCount ?? 0),
      0,
    );
  ...
  const filteredDoctors = useMemo(() => {
    ...
    return safeDoctors.filter((d) => {
      ...
      if ((d?.fee || 0).toString().includes(q)) return true;

      if (!Number.isNaN(qNum) && (d?.fee || 0) <= qNum) return true;
