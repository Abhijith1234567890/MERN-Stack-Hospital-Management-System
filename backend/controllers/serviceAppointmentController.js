...
export const createServiceAppointment = async (req, res) => {
  try {
    ...
    console.error("createServiceAppointment unexpected:", error);
...
// to getSeviceAppointmentById
export const getSeviceAppointmentById = async (req, res) => {
  try {
    const { id } = req.params
    ...
    console.error("getSeviceAppointmentById error:", error);
    return res.status(500).json({ success: false, message: "Server error" });
  }
}
...
export const updateServiceAppointment = async (req, res) => {
  try {
    ...
    // check whether fill if yes then update the field
...
export const getServiceAppointmentStats = async (req, res) => {
  try {
    const services = await Service.aggregate([
      ...
      { $sort: { createdAt: -1 } },
    ]); // it will give you total appoitment, complete appt
...
export const getServiceAppointmentsByPatient = async (req, res) => {
  try {
    ...
    const appointments = await ServiceAppointment.find(filter)
      .populate("serviceId", "name image imageUrl")
      .sort({ createdAt: -1 })
      .lean();
